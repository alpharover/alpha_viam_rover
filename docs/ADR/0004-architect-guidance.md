...
3. **Spawner+param‑file edge cases.**
   The **controller spawner** wants the param file **on the machine that runs `controller_manager`**, and on Humble it’s easier to trip over YAML shapes/namespacing. In newer releases the spawner grew better wildcard support and multi‑file parsing, but Humble is touchier. Treat the param‑file path as local-to-manager, and if you still see misapplied params, use a **shim** that loads, sets parameters via service, then activates. There’s even a fresh issue noting the spawner’s parameter-file locality assumption. ([GitHub][4], [control.ros.org][5])
