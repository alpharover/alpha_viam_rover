...
def check_controllers_yaml(p: Path) -> list[str]:
    ...
    if 'params_file:' in txt:
        issues.append("controllers.yaml: avoid manager-side 'params_file'; use spawner --param-file instead")
    ...
    if 'diff_drive_controller:' in txt and 'left_wheel_names:' not in txt:
        issues.append("controllers.yaml: diff_drive_controller missing inline params; provide them or --param-file with correct YAML")
    return issues
...
def check_controller_params(p: Path) -> list[str]:
    ...
    if not (txt.startswith('/**:') or txt.startswith('diff_drive_controller:')):
        issues.append(f"{p.name}: expected '/**:' or 'diff_drive_controller:' root with 'ros__parameters' block")
