...
def test_exclude_file(
    ...
    bad_name.write_bytes((combinations + "5 abandonned 5\n6 abandonned 6").encode("utf-8"))
...
def test_case_handling_in_fixes(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_context(
    ...
    (tmp_path / "context.txt").write_text("line 1\nline 2\nline 3 abandonned\nline 4\nline 5")
...
def test_uri_regex_option(
    ...
    assert cs.main(fname, "--uri-regex=\\bhttp.*\\b", "--uri-ignore-words-list=abandonned") == 0
...
def test_stdin(tmp_path: Path) -> None:
    ...
    for single_line_per_error in (True, False):
        ...
        assert run_codespell_stdin(text, args=args, cwd=tmp_path) == input_file_lines * (
