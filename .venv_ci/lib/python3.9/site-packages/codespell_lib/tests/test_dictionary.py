...
_fnames_in_aspell = [
    (op.join(_data_dir, f"dictionary{d[2]}.txt"), d[3:5], d[5:7]) for d in _builtin_dictionaries
]
...
def _check_err_rep(
    ...
    del msg
    if rep.count(","):
        assert rep.endswith(","), f'error {err}: multiple corrections must end with trailing ","'
...
def test_dictionary_looping(
    ...
    with open(fname, encoding="utf-8") as fid:
        for line in fid:
            err, rep = line.split("->")
            err = err.lower()
            assert err not in this_err_dict, f"error {err!r} already exists in {short_fname}"
            rep = rep.rstrip("\n")
