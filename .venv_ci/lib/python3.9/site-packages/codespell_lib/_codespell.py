...
uri_regex_def = "(\\b(?:https?|[ts]?ftp|file|git|smb)://[^\\s]+(?=$|\\s)|\\b[\\w.%+-]+@[\\w.-]+\\b)"
...
def parse_options(
    ...
    builtin_opts = "\n- ".join([""] + [f"{d[0]!r} {d[1]}" for d in _builtin_dictionaries])
    parser.add_argument(
        "--builtin",
        dest="builtin",
...
def build_ignore_words(filename: str, ignore_words: Set[str], ignore_words_cased: Set[str]) -> None:
    with open(filename, encoding="utf-8") as f:
        process_ignore_words((line.strip() for line in f), ignore_words, ignore_words_cased)


def is_hidden(filename: str, check_hidden: bool) -> bool:
    bfilename = os.path.basename(filename)

    return bfilename not in ("", ".", "..") and (not check_hidden and bfilename[0] == ".")
...
def parse_file(
    ...
    for i, line in enumerate(lines):
        ...
        if match:
            extra_words_to_ignore = set(filter(None, (match.group("words") or "").split(",")))
        ...
        for match in check_matches:
            ...
            if lword in misspellings and lword not in extra_words_to_ignore:
                ...
                if (not context_shown) and (context is not None):
                    print_context(lines, i, context)
                if filename != "-":
                    print(f"{cfilename}:{cline}: {cwrongword} ==> {crightword}{creason}")
                elif options.stdin_single_line:
                    print(f"{cline}: {cwrongword} ==> {crightword}{creason}")
                else:
                    print(f"{cline}: {line.strip()}\n\t{cwrongword} " f"==> {crightword}{creason}")
...
def flatten_clean_comma_separated_arguments(
    ...
    []
    """
    return [item.strip() for argument in arguments for item in argument.split(",") if item]
...
def main(*args: str) -> int:
    ...
    ignore_words, ignore_words_cased = parse_ignore_words_option(options.ignore_words_list)
    if options.ignore_words:
        ignore_words_files = flatten_clean_comma_separated_arguments(options.ignore_words)
    ...
    try:
        ...
        return _usage_error(
            parser,
            "ERROR: --skip/-S has been fed an invalid glob, " "try escaping special characters",
        )
        ...
        if os.path.isdir(filename):
            for root, dirs, files in os.walk(filename):
                ...
                dirs[:] = [
                    dir_
                    for dir_ in dirs
                    if not glob_match.match(dir_) and not is_hidden(dir_, options.check_hidden)
                ]
