...
def supports_color():
    supported_platform = not (
        ...
        and not ("ANSICON" in os.environ or ("TERM" in os.environ and os.environ["TERM"] == "ANSI"))
    )
...
class Format:
    ...
    def parsable(problem, filename):
        return (
            f"{filename}:{problem.line}:{problem.column}: " f"[{problem.level}] {problem.message}"
        )
...
def run(argv=None):
    ...
    parser.add_argument(
        "--list-files", action="store_true", dest="list_files", help="list files to lint and exit"
    )
    parser.add_argument(
        "-f",
        "--format",
    ...
    # User-global config is supposed to be in ~/.config/yamllint/config
        ...
        user_global_config = os.path.join(os.environ["XDG_CONFIG_HOME"], "yamllint", "config")
    else:
