...
def check(conf, line):
    if line.start == line.end and line.end < len(line.buffer):
        ...
        if line.end + 2 <= len(line.buffer) and line.buffer[line.end : line.end + 2] == "\n\n":
            ...
            line.end + 4 <= len(line.buffer) and line.buffer[line.end : line.end + 4] == "\r\n\r\n"
        ):
            return
        ...
        if (line.end == len(line.buffer) - 1 and line.buffer[line.end] == "\n") or (
            line.end == len(line.buffer) - 2 and line.buffer[line.end : line.end + 2] == "\r\n"
        ):
        ...
        if blank_lines > max:
            yield LintProblem(line.line_no, 1, f"too many blank lines ({blank_lines} > {max})")
