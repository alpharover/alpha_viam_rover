...
def check(conf, token, prev, next, nextnext, context):
    if isinstance(token, yaml.ValueToken) and not (
        isinstance(prev, yaml.AliasToken) and token.start_mark.pointer - prev.end_mark.pointer == 1
    ):
        ...
        problem = spaces_after(
            token, prev, next, max=conf["max-spaces-after"], max_desc="too many spaces after colon"
        )
        if problem is not None:
            yield problem
