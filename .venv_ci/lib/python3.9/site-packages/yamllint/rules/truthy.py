...
TRUTHY_1_2 = ["TRUE", "True", "true", "FALSE", "False", "false"]
    ...
    if isinstance(token, yaml.tokens.ScalarToken) and token.style is None:
        if "bad_truthy_values" not in context:
            context["bad_truthy_values"] = set(
                TRUTHY_1_2 if yaml_spec_version_for_document(context) == (1, 2) else TRUTHY_1_1
            )
        ...
        if token.value in context["bad_truthy_values"]:
            yield LintProblem(
                ...
                "truthy value should be one of [" + ", ".join(sorted(conf["allowed-values"])) + "]",
            )
