...
def check(conf, token, prev, next, nextnext, context):
    if (
        ...
        or conf["forbid-duplicated-anchors"]
        or conf["forbid-unused-anchors"]
    ):
        if isinstance(
            token, (yaml.StreamStartToken, yaml.DocumentStartToken, yaml.DocumentEndToken)
        ):
            context["anchors"] = {}

    if (
        conf["forbid-undeclared-aliases"]
        and isinstance(token, yaml.AliasToken)
        and token.value not in context["anchors"]
    ):
        yield LintProblem(
    ...
    if (
        ...
        and isinstance(token, yaml.AnchorToken)
        and token.value in context["anchors"]
    ):
        yield LintProblem(
            token.start_mark.line + 1,
            token.start_mark.column + 1,
    ...
    if conf["forbid-unused-anchors"]:
        ...
        if isinstance(next, (yaml.StreamEndToken, yaml.DocumentStartToken, yaml.DocumentEndToken)):
            for anchor, info in context["anchors"].items():
                if not info["used"]:
                    yield LintProblem(
                        info["line"] + 1, info["column"] + 1, f'found unused anchor "{anchor}"'
                    )
