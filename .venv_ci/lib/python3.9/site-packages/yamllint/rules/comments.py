...
ID = "comments"
TYPE = "comment"
CONF = {"require-starting-space": bool, "ignore-shebangs": bool, "min-spaces-from-content": int}
DEFAULT = {"require-starting-space": True, "ignore-shebangs": True, "min-spaces-from-content": 2}
...
def check(conf, comment):
    if (
        ...
        yield LintProblem(
            comment.line_no,
            comment.column_no,
            "too few spaces before comment: expected " f'{conf["min-spaces-from-content"]}',
        )
    ...
    if conf["require-starting-space"]:
        ...
        if text_start < len(comment.buffer):
            ...
            # \r itself is a valid newline on some older OS.
                ...
                yield LintProblem(comment.line_no, column, "missing starting space in comment")
