...
def spaces_after(token, prev, next, min=-1, max=-1, min_desc=None, max_desc=None):
    if next is not None and token.end_mark.line == next.start_mark.line:
        ...
        if max != -1 and spaces > max:
            return LintProblem(token.start_mark.line + 1, next.start_mark.column, max_desc)
        elif min != -1 and spaces < min:
            return LintProblem(token.start_mark.line + 1, next.start_mark.column + 1, min_desc)
...
def spaces_before(token, prev, next, min=-1, max=-1, min_desc=None, max_desc=None):
    if (
        ...
        (prev.end_mark.pointer == 0 or prev.end_mark.buffer[prev.end_mark.pointer - 1] != "\n")
    ):
        spaces = token.start_mark.pointer - prev.end_mark.pointer
        if max != -1 and spaces > max:
            return LintProblem(token.start_mark.line + 1, token.start_mark.column, max_desc)
        elif min != -1 and spaces < min:
            return LintProblem(token.start_mark.line + 1, token.start_mark.column + 1, min_desc)
...
def get_real_end_line(token):
    ...
    while pos >= token.start_mark.pointer - 1 and token.end_mark.buffer[pos] in string.whitespace:
