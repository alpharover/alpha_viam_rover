...
def _has_quoted_quotes(token):
    return (not token.plain) and (
        (token.style == "'" and '"' in token.value) or (token.style == '"' and "'" in token.value)
    )
...
def _has_backslash_on_at_least_one_line_ending(token):
    if token.start_mark.line == token.end_mark.line:
        return False
    buffer = token.start_mark.buffer[token.start_mark.index + 1 : token.end_mark.index - 1]
    return "\\\n" in buffer or "\\\r\n" in buffer
...
def check(conf, token, prev, next, nextnext, context):
    ...
    if conf["required"] is True:
        ...
        if (
            ...
            is_extra_required = any(re.search(r, token.value) for r in conf["extra-required"])
        ...
        if (
            ...
            and token.value
            and not _quotes_are_needed(token, context["flow_nest_count"] > 0)
        ):
            is_extra_required = any(re.search(r, token.value) for r in conf["extra-required"])
            is_extra_allowed = any(re.search(r, token.value) for r in conf["extra-allowed"])
            if not (is_extra_required or is_extra_allowed):
                msg = f"string {node} is redundantly quoted with " f"{quote_type} quotes"
