...
def check(conf, token, prev, next, nextnext, context):
    ...
    if conf["forbid-in-block-mappings"]:
        if isinstance(token, yaml.ValueToken) and isinstance(
            ...
            yield LintProblem(
                token.start_mark.line + 1, token.end_mark.column + 1, "empty value in block mapping"
            )
    ...
    if conf["forbid-in-flow-mappings"]:
        if isinstance(token, yaml.ValueToken) and isinstance(
            ...
            yield LintProblem(
                token.start_mark.line + 1, token.end_mark.column + 1, "empty value in flow mapping"
            )
