...
class YamlLintConfig:
    ...
    def enabled_rules(self, filepath):
        return [
            ...
            if val is not False
            and (filepath is None or "ignore" not in val or not val["ignore"].match_file(filepath))
        ]
    ...
    def parse(self, raw_content):
        ...
        if "ignore" in conf and "ignore-from-file" in conf:
            ...
            self.ignore = pathspec.PathSpec.from_lines(
                "gitwildmatch", decoder.lines_in_files(conf["ignore-from-file"])
            )
        elif "ignore" in conf:
    ...
    if isinstance(conf, dict):
        ...
        for optkey in conf:
            ...
            if optkey not in options:
                raise YamlLintConfigError(
                    f'invalid config: unknown option "{optkey}" for rule ' f'"{rule.ID}"'
                )
        ...
        raise YamlLintConfigError(
            f'invalid config: rule "{rule.ID}": should be either "enable", ' f'"disable" or a dict'
        )
